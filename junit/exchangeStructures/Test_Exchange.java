...
public class Test_Exchange extends junit.framework.TestCase {
	...
	public void test1() throws Exception {
		...
		// ++ here we used ClientOrderId as key to get a specific order, orders might be stored as a map in exchange.
		...
		assertTrue( 
			exchange
				...
				.getBidBook()
				.getPriceLevels()
				.get( priceOfPriceLevel )      // ++ here we see priceLevels might be a ordered map using Price as key
				.getOrders()				   // "++" means the comments that I added when reading this test.
				.get( orderIndex )			   // ++ since we used orderIndex to get the first order, then orders might be a list 
				.equals( new RestingOrder( sweepingOrder ) )  // ++ if random access is frequent, then arrayList is better than linkedList
		);
		...
		exchange.getComms().getRestingOrderConfirmations().removeLast();  // ++ RestingOrderConfimations should be a list
		...
		// There should be no matches. Furthermore, the sweep
		// should see a cancelled order and remove it from the
		// book. As there should be no additional orders at the
		// same price level, the price level should also be
		// removed
		...
		// Make sure the sweep removed the cancelled order and the empty
		// price level
